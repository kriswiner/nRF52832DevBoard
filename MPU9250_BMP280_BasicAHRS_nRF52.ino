/* MPU9250_BMP280_BasicAHRS_nRF52 Basic Example Code
 by: Kris Winer
 date: March 15, 2017
 ...
 Mahony filter algorithms. Sketch runs on the 3.3 V 64 MHz nRF52832 Development Board.
 
 SDA and SCL should have external pull-up resistors (to 3.3V).
 4K7 resistors are on the MPU9250+BMP280 breakout board.
 
 Hardware setup:
 MPU9250 Breakout --------- nRF52
 VDD ---------------------- 3.3V
 SDA ----------------------- 6 
 SCL ----------------------- 7
 GND ---------------------- GND
...
float   MPU9250gyroBias[3] = {0, 0, 0}, MPU9250accelBias[3] = {0, 0, 0}, MPU9250magBias[3] = {0, 0, 0}, MPU9250magScale[3]  = {0, 0, 0};      // Bias corrections for gyro and accelerometer
...
void setup()
{
 ...
 // Environmental Sensor service
  ...
  Serial.println(F("BLE MPU9250+BMP280 Peripheral"));
...
//      bleSerial.poll();
  ...
  // If intPin goes high, all data registers have new data
   if(newData == true) {  // On interrupt, read data
     ...
     MPU9250readData(MPU9250Data); // INT cleared on any read
    
    // Now we'll calculate the accleration value into actual g's
    ax = (float)MPU9250Data[0]*MPU9250aRes - MPU9250accelBias[0];  // get actual g value, this depends on scale being set
    ay = (float)MPU9250Data[1]*MPU9250aRes - MPU9250accelBias[1];   
    az = (float)MPU9250Data[2]*MPU9250aRes - MPU9250accelBias[2];  

    // Calculate the gyro value into actual degrees per second
    gx = (float)MPU9250Data[4]*MPU9250gRes;  // get actual gyro value, this depends on scale being set
    gy = (float)MPU9250Data[5]*MPU9250gRes;  
    gz = (float)MPU9250Data[6]*MPU9250gRes;   
    ...
    if(newMagData == true) { // wait for magnetometer data ready bit to be set
      ...
      mx *= MPU9250magScale[0];
      my *= MPU9250magScale[1];
...
//  MahonyQuaternionUpdate(ay, ax, -az, gy*pi/180.0f, gx*pi/180.0f, -gz*pi/180.0f,  mx,  my, mz);
    ...
    setElevationCharacteristicValue();
    ...
    battlevelCharacteristic.setValue((uint8_t)battlevel);
...
void MPU9250getMres() {
  switch (MPU9250Mscale)
  {
    ...
    case MFS_14BITS:
          MPU9250mRes = 10.0f*4912.0f/8190.0f; // Proper scale to return milliGauss
          break;
    case MFS_16BITS:
          MPU9250mRes = 10.0f*4912.0f/32760.0f; // Proper scale to return milliGauss
          break;
  }
}
...
void MPU9250getGres() {
  switch (MPU9250Gscale)
  {
    ...
    case GFS_250DPS:
          MPU9250gRes = 250.0f/32768.0f;
          break;
    case GFS_500DPS:
          MPU9250gRes = 500.0f/32768.0f;
          break;
    case GFS_1000DPS:
          MPU9250gRes = 1000.0f/32768.0f;
          break;
    case GFS_2000DPS:
          MPU9250gRes = 2000.0f/32768.0f;
          break;
  }
}
...
void MPU9250getAres() {
  switch (MPU9250Ascale)
  {
	...
	// 2 Gs (00), 4 Gs (01), 8 Gs (10), and 16 Gs  (11). 
        // Here's a bit of an algorith to calculate DPS/(ADC tick) based on that 2-bit value:
    case AFS_2G:
          MPU9250aRes = 2.0f/32768.0f;
          break;
    case AFS_4G:
          MPU9250aRes = 4.0f/32768.0f;
          break;
    case AFS_8G:
          MPU9250aRes = 8.0f/32768.0f;
          break;
    case AFS_16G:
          MPU9250aRes = 16.0f/32768.0f;
          break;
  }
}

void MPU9250readData(int16_t * destination)
{
...
{  
	...
	Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
	uint8_t i = 0;
        Wire.requestFrom(address, count);  // Read bytes from slave register address         Wire.requestFrom(address, (size_t) count);  // Read bytes from slave register address 
